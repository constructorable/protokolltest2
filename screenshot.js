...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.getElementById('screenshot').addEventListener('click', async function () {
        ...
        /*         // NEUE BESTÄTIGUNGSLOGIK FÜR DEN CLOSE-BUTTON
                closeButton.onclick = function() {
                    // Bestätigungsdialog anzeigen
                    if (confirm('hiermit wird das Popup-Fenster geschlossen')) {
                        closeModal();
                    }
                    // Wenn "Abbrechen" geklickt wird, passiert nichts und das Modal bleibt offen
                }; */

        closeButton.onclick = function () {
            // Prüfen ob bereits ein Confirmation Modal existiert
            const existingModal = document.querySelector('.confirmation-modal');
            if (existingModal) {
                existingModal.remove();
            }

            // Prüfen ob bereits Styles existieren und diese entfernen
            const existingStyle = document.querySelector('#confirmation-modal-styles');
            if (existingStyle) {
                existingStyle.remove();
            }

            // Custom Confirmation Modal erstellen
    ...
    `;
            ...
            // CSS Styles hinzufügen mit eindeutiger ID
            const style = document.createElement('style');
            style.id = 'confirmation-modal-styles'; // Eindeutige ID für spätere Identifikation
            style.textContent = `
        .confirmation-modal {
            position: fixed;
            top: 0;
    ...
    `;
            ...
            // Style und Modal zum DOM hinzufügen
            ...
            const handleConfirm = () => {
                cleanup();
                // Kurze Verzögerung bevor closeModal() aufgerufen wird
                setTimeout(() => {
                    closeModal(); // Ihr ursprünglicher closeModal() Aufruf
                }, 100);
            };

            // Event Listener hinzufügen (einmalig)
            cancelBtn.onclick = handleCancel;
            confirmBtn.onclick = handleConfirm;

            // Modal schließen bei Hintergrund-Klick
...
`;
    ...
    document.getElementById('screenshot')?.addEventListener('click', async () => {
        try {
            ...
            // Automatischer Download (Browser entspeichert im Standard-Download-Ordner)
